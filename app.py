...
def apply_scale_factor_to_raw_data(data_nested, unit):
    ...
    if not isinstance(data_nested, dict):
        # Si es un valor numérico directo, escalarlo y devolverlo.
        if isinstance(data_nested, (int, float)):
            return data_nested * scale_factor
        return data_nested # Devolverlo tal cual si no es un diccionario ni un número
...
def map_and_aggregate_balance(raw_balance_data_nested, synonyms_map, unit):
    ...
    # Si scaled_raw_data no es un diccionario (es un valor directo o nulo)
    if not isinstance(scaled_raw_data, dict):
        st.warning(f"Advertencia: Datos escalados de Balance General no son un diccionario: {type(scaled_raw_data)}. Intentando mapear como un total global.")
        ...
        # Buscamos la clave original para saber qué total es (ej. "TOTAL ACTIVOS").
        if isinstance(raw_balance_data_nested, dict) and len(raw_balance_data_nested) == 1:
            ...
            if mapped_name and mapped_name in aggregated_data_final:
                if isinstance(scaled_raw_data, (int, float)):
                    aggregated_data_final[mapped_name] = scaled_raw_data # Asignar el total global
                else:
                    st.error(f"Error: Total global mapeado '{mapped_name}' no es un número después de escalar: {type(scaled_raw_data)}")
            else:
